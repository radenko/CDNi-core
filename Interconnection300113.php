...
class Interconnection {
    ...
    function __construct($config) {
        $this -> config = $config;  //Put all information from array in config.php into local $config variable     
        $this -> db = new DB($this->config['i']); //Put information from array in array in config.php into new database DB
    }
    ...
    function processLocalOffers() {        
        if (isset($this -> config['i']['peers'])) { 
            foreach ($this -> config['i']['peers'] as $peer) {
                ...
                $this -> db -> select('interconnections','COUNT(*)',array('WHERE' => "peerURL='". $this->db->escape_string($peer['APIurl']). "'")); //Where does the table "interconnections" gets defined?

                if ($this -> db -> errno()) echo $this -> db -> error (); //Checks whether database connection is specified?
                elseif ($this -> db -> result() <= 0) /*Check whether database empty? */ {
                    $this -> addPeer($peer); //Call addPeer with $config['CDN']['i']['peers'] from config.php
